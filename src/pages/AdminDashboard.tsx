...
import { getPlants, getLines, getLocations, LOCATION_PASSWORDS, ADMIN_PORTAL_PASSWORD } from "@/lib/constants";
...
const AdminDashboard = () => {
  ...
  const handleLocationChange = (value: string) => {
    if (value === "all") {
      const locked = locations.filter((l: string) => !unlockedLocations.includes(l));
      if (locked.length > 0) {
  ...
  const handleExcelClick = () => exportToExcel();
      ...
      <div className="sticky top-0 z-30 bg-background border-b shadow-sm">
        ...
        <header className="bg-card px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" variant="outline" onClick={handleAdminPortalClick}>
        ...
        <div className="px-4 py-2 bg-card/80 backdrop-blur flex flex-wrap items-center gap-2 border-t">
          ...
          <Select value={filterLocation} onValueChange={handleLocationChange}>
            <SelectTrigger className="w-[150px] h-8 text-xs"><SelectValue placeholder="Location" /></SelectTrigger>
            <SelectContent>
          ...
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-1">
                {locations.filter((l: string) => l !== filterLocation).map((l: string) => (
        ...
        <DialogContent>
          ...
          <div className="space-y-4 pt-2">
            <div className="space-y-1.5">
              ...
              <Input type="password" value={locationPassword} onChange={(e) => { setLocationPassword(e.target.value); setPasswordError(""); }} onKeyDown={(e) => e.key === "Enter" && handleUnlockLocation()} placeholder="Enter location password" />
            </div>
            {passwordError && <p className="text-sm text-destructive">{passwordError}</p>}
            <Button onClick={handleUnlockLocation} className="w-full bg-primary text-primary-foreground">Unlock</Button>
          </div>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 pt-2">
            <div className="space-y-1.5">
              ...
              <Input type="password" value={excelPassword} onChange={(e) => { setExcelPassword(e.target.value); setExcelPasswordError(""); }} onKeyDown={(e) => e.key === "Enter" && handleExcelUnlock()} placeholder="Enter location password" />
            </div>
            {excelPasswordError && <p className="text-sm text-destructive">{excelPasswordError}</p>}
            <Button onClick={handleExcelUnlock} className="w-full bg-primary text-primary-foreground">Export</Button>
          </div>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 pt-2">
            <div className="space-y-1.5">
              ...
              <Input type="password" value={portalPassword} onChange={(e) => { setPortalPassword(e.target.value); setPortalPasswordError(""); }} onKeyDown={(e) => e.key === "Enter" && handlePortalUnlock()} placeholder="Enter portal password" />
            </div>
            {portalPasswordError && <p className="text-sm text-destructive">{portalPasswordError}</p>}
            <Button onClick={handlePortalUnlock} className="w-full bg-primary text-primary-foreground">Access Portal</Button>
            <p className="text-xs text-center text-muted-foreground">Password: portal@admin123</p>
          </div>
        </DialogContent>
      </Dialog>
    </div>
